...
function allowableBrakingTorque (tb_curveAbove, ed, brakeTime) {
  ...
  const closestPointCurveBelow = [tb_curveBelow.brakeTime[timeIndexCurveBelow - 1], tb_curveBelow.dutyCycle[timeIndexCurveBelow - 1]];
  ...
  if ((brakeCurveSegment.slope * point1_x + brakeCurveSegment.yCrossing) - point1_y > point1_y * 0.05) console.log(`Calculation error`);
  ...
  if ((brakeCurveSegment.slope * point2_x + yCrossParLine) - point2_y > point2_y * 0.05) console.log(`Calculation error`);
  ...
  // (7) Find first point from (6) with braking time and ED% higher than operation point
}